...
If not exist %dir%\jenkins-cli.jar (
     ...
     REM curl http://%Host%:8080/jnlpJars/jenkins-cli.jar -O
         curl https://github.com/gajananchoundhe/Mytest/blob/master/jenkins-cli.jar -O
...
REM java -jar jenkins-cli.jar -s http://%Host%:8080/ build TestJob1  -s -v -p browserName=%Browser% -p jobID=%JOBID%
...
Echo Client node connecting to server, please wait moment
TIMEOUT /T 10
